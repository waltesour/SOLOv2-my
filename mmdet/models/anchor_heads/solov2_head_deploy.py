...
class SOLOv2Head(nn.Module):
    ...
    def get_seg_single(self,
                       cate_preds,  # cate_preds.shape = [3872,80]
                       seg_preds,   # seg_preds.shape = [3872,80]
                       kernel_preds,# kernel_preds.shape = [1,128,112,168],其中112,168是mask Feature F的尺寸
                       featmap_size,# featmap_size = torch.Size([112,168])
                       img_shape,   # 根据配置文件缩放后的尺寸
                       ori_shape,
                       scale_factor,
                       cfg,
        ...
        # process.
        inds = (cate_preds > cfg.score_thr) # 将PFN每个feature map上的每个grid(共3872个)中的80类中,类别置信度大于thr的,对应索引处inds为true
        cate_scores = cate_preds[inds] #cate_scores是提取出来的符合条件的类别置信度 k个
        if len(cate_scores) == 0:
            return None
        '''将符合置信度的目标所在grid对应的卷积核信息 kernel_preds 、以及目标所属标签cate_labels 提取出来'''
        # cate_labels & kernel_preds
        inds = inds.nonzero() # inds是二维:一维是符合置信度的k个目标所在的grid索引,二维是对应grid索引上目标类别的编号(0-79)
        cate_labels = inds[:, 1] # cate_labels是提取出来的目标标签
        kernel_preds = kernel_preds[inds[:, 0]] # 将符合置信度的目标所在grid的 kernel提取出来
        '''将符合置信度的目标所在grid对应的 strides 提取出来'''
        # trans vector.
        size_trans = cate_labels.new_tensor(self.seg_num_grids).pow(2).cumsum(0) # size_trans=tensor([1600,2896,3472,3728,3872]),将[40, 36, 24, 16, 12]求平方后递增求和
        strides = kernel_preds.new_ones(size_trans[-1])  # new_ones返回一个与size_trans[-1]大小相同的用1填充的张量

        n_stage = len(self.seg_num_grids) # n_stage 是FPN输出的feature map数目
        strides[:size_trans[0]] *= self.strides[0]
        for ind_ in range(1, n_stage):
            strides[size_trans[ind_-1]:size_trans[ind_]] *= self.strides[ind_]
        strides = strides[inds[:, 0]] # 将符合置信度的目标所在grid对应的 strides 提取出来

        # mask encoding.
        I, N = kernel_preds.shape
        kernel_preds = kernel_preds.view(I, N, 1, 1)
        # seg_preds.shape = [1,128,112,168] seg_preds是为所有FPN级别预测的一个统一的 F
        # kernel_preds.shape = [360,128,1,1] 说明本次输入图像中有360个grid处的1×1卷积被激活,每个卷积通道数为128
        # 卷积后得到 seg_preds.shape = [1,360,112,168],得到了360个实例特征图
        seg_preds = F.conv2d(seg_preds, kernel_preds, stride=1).squeeze(0).sigmoid() #squeeze(0)去掉第0维
        # mask.
        seg_masks = seg_preds > cfg.mask_thr #使用阈值0.5将预测的soft mask转变为二值图
        sum_masks = seg_masks.sum((1, 2)).float() # 将每个特征图上的true求和

        # filter.-每个实例的特征尺寸如果比其所在grid对应的 strides还小,就过滤掉,因为这种特征在提取特征过程中就丢失了
        keep = sum_masks > strides 
        if keep.sum() == 0:
            return None

        seg_masks = seg_masks[keep, ...] #seg_masks是实例的像素位置
        seg_preds = seg_preds[keep, ...] #seg_preds包含每个实例上每个像素位置的conf
        sum_masks = sum_masks[keep]
        cate_scores = cate_scores[keep]
        cate_labels = cate_labels[keep]

        # maskness.
        seg_scores = (seg_preds * seg_masks.float()).sum((1, 2)) / sum_masks # 像素conf均值
        cate_scores *= seg_scores #将分割的像素conf均值与类别conf相乘,作为新的类别conf

        # sort and keep top nms_pre 保留前cfg.nms_pre 个
        sort_inds = torch.argsort(cate_scores, descending=True) # 降序
