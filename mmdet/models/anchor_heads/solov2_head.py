...
class SOLOv2Head(nn.Module):
    ...
    def forward(self, feats, eval=False):
        ...
        cate_pred, kernel_pred = multi_apply(self.forward_single, new_feats, #将split_feats()函数返回的特征图，依次使用forward_single()函数进行计算
                                                       list(range(len(self.seg_num_grids))),
                                                       eval=eval, upsampled_size=upsampled_size)
        return cate_pred, kernel_pred # kernel与category branch位于同一个预测头中
    ...
    def get_seg(self, cate_preds, kernel_preds, seg_pred, img_metas, cfg, rescale=None):
        ...
        for img_id in range(len(img_metas)):
            ...
            kernel_pred_list = [
                kernel_preds[i][img_id].permute(1, 2, 0).contiguous().view(-1, self.kernel_out_channels).detach()
                                for i in range(num_levels) #新增了.contiguous()
            ]
    ...
    def get_seg_single(self,
        ...
        assert len(cate_preds) == len(kernel_preds)  # 判断类别预测与kernel预测除了通道数不同，其他尺寸是相同的，因为是一个head出来的
