...
# 将tensor类型的变量保存下来
def save_image_tensor2cv(input_tensor,filename):
    assert (len(input_tensor.shape) == 4 and input_tensor.shape[0] == 1)
    # 复制一份
    input_tensor = torch.from_numpy(input_tensor).clone().detach()
    # 到cpu
    ...
    for index in range(0,input_tensor.shape[0]):
        input_part = input_tensor[index].clone().detach()
        input_tensor2 = input_part.unsqueeze(0).mul_(255).add_(0.5).clamp_(0, 255).permute(1, 2, 0).type(torch.uint8).numpy()
        # RGB 转 BRG
        input_tensor2 = cv2.cvtColor(input_tensor2, cv2.COLOR_RGB2BGR)
        cv2.imwrite(filename[:-4]+str(index)+'.jpg', input_tensor2)
...
def convert2onnx(args, dummy_input):
    ...
    if hasattr(model, 'forward_dummy'): #hasattr() 函数用于判断对象是否包含对应的属性
...
def check(args, dummy_input, check_onnx=True, check_trt=True):
    ...
    with torch.no_grad():
        torch_output = model(dummy_input) #python直接输出的结果
    ...
    if check_onnx:
        ...
        onnx_output = sess.run(output_names, {input_names[0]: dummy_input.cpu().numpy()}) #调用onnx输出的结果
        # save_image_tensor2cv(onnx_output[2],"cate.jpg")
        print("onnxruntime")
